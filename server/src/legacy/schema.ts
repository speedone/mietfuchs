// Das Schema, wie es nach Migration 0000 aussieht. **Eingefroren.**
//
// Diese Datei wird nie geändert. Sie beschreibt den Stand, auf den der Import einer alten
// db.json zielt, und der Stand ist per Definition der von damals. Wer das Schema ändert, ändert
// [../db/schema.ts](../db/schema.ts) und erzeugt einen Migrationsschritt; dass ein alter Bestand
// danach auch dort ankommt, erledigt die Migrationskette, die im Umstieg über den übernommenen
// Bestand läuft (db/changeover.ts). Die Regeln dieser Kette stehen in
// [../../drizzle/README.md](../../drizzle/README.md) und gelten für diese Datei mit.
//
// **Warum sie nicht einfach db/schema.ts benutzt.** Weil die dort mitwandert. Der Import zielte
// dann wieder auf den neuesten Stand, und jede Regel, die den Weg dorthin beschreibt, müsste ein
// zweites Mal hier stehen. Genau das soll die Aufteilung beenden. Bei Django heißt das Muster
// „historische Modelle", bei Flyway „Baseline"; die Begründung im Langen steht in
// [README.md](README.md) neben dieser Datei.
//
// **Darum auch kein Import aus `shared/types.ts`.** Die Listen der Aufzählungen stehen hier als
// schlichte Zeichenketten und sind bewusst **nicht** an die Domänentypen gebunden, wie sie es in
// db/schema.ts sind. Eine Bindung an einen lebenden Typ wäre das Gegenteil von eingefroren: Käme
// dort ein Wert hinzu oder hieße einer anders, wanderte diese Datei mit, und der Wortschatz von
// damals wäre verloren.
//
// Dass die Kopie wirklich dem entspricht, was Migration 0000 anlegt, hält
// [../../test/legacy-schema.test.ts](../../test/legacy-schema.test.ts) fest: Er legt eine
// Datenbank mit genau diesem einen Schritt an und vergleicht Tabellen und Spalten.
//
...
// Der Wortschatz von damals. Nicht an die Domänentypen gebunden, siehe den Kopf: Eine Bindung an
// einen lebenden Typ wäre das Gegenteil von eingefroren.
