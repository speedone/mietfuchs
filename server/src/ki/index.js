...
//
//   json({ prompt, bilder, schema, timeoutMs }) → Promise<object>
//
// `bilder` ist eine Liste von { mimeType, data } mit den Bilddaten als Base64 in `data`, leer
...
import { ollamaAnbieter } from './ollama.js'

export function kiAnbieter(settings) {
  return ollamaAnbieter(settings)
}
