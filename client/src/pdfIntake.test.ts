...
function scheitert(name: string, message: string, beim: 'open' | 'text' | 'pages' = 'open') {
  const fehler = Object.assign(new Error(message), { name })
  const doc: OpenedPdf = {
    text: vi.fn(async () => { if (beim === 'text') throw fehler; return '' }),
    pages: vi.fn(async () => { throw fehler }),
    close: vi.fn(),
  }
  const reader: PdfReader = { open: vi.fn(async () => { if (beim === 'open') throw fehler; return doc }) }
  return { reader, doc }
}
...
describe('Beleg vor dem Hochladen vorbereiten', () => {
  ...
  test('Scan: höchstens vier Seiten gehen als Bilder mit, das PDF wird nur einmal geöffnet', async () => {
    ...
    const { reader, doc } = leser('Seite 1', 6)
    const fd = await buildUpload(pdf(), reader)
  ...
  test('die Schwelle für brauchbaren Text liegt bei genau 80 Zeichen, ohne Leerraum', async () => {
    ...
    const genug = await buildUpload(pdf(), leser(`  ${'x'.repeat(80)}  `, 2).reader)
    expect(genug.getAll('pages')).toHaveLength(0)
    const zuWenig = await buildUpload(pdf(), leser('x'.repeat(79), 2).reader)
    expect(zuWenig.getAll('pages')).toHaveLength(2)
  })

  test('sehr langer Text wird gekürzt, bevor er hochgeht', async () => {
    const fd = await buildUpload(pdf(), leser('a'.repeat(PDF_TEXT_MAX + 5000)).reader)
  ...
  test('ein PDF ohne Seiten wird gemeldet statt leer hochgeschickt', async () => {
    await expect(buildUpload(pdf(), leser('', 0).reader)).rejects.toThrow('Das PDF enthält keine Seiten.')
  })
})

describe('Meldungen, wenn ein PDF nicht lesbar ist', () => {
  test('passwortgeschützt', async () => {
    await expect(buildUpload(pdf(), scheitert('PasswordException', 'No password given').reader)).rejects.toThrow(
  ...
  test('beschädigt oder gar kein PDF', async () => {
    await expect(buildUpload(pdf(), scheitert('InvalidPDFException', 'Invalid PDF structure.').reader)).rejects.toThrow(
      'Die Datei ist kein gültiges PDF oder beschädigt.',
    )
  })

  test('eine Seite lässt sich nicht als Bild speichern', async () => {
    const { reader, doc } = scheitert('SeiteAlsBild', 'Eine Seite ließ sich nicht als Bild speichern.', 'pages')
  ...
  test('sonstige Fehler nennen den Grund', async () => {
    const { reader, doc } = scheitert('UnknownErrorException', 'Worker was destroyed', 'text')
