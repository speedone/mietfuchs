...
lokale **Ollama**-Instanz (URL/Modell aus den Settings). PDF → Textebene via `pdf-parse` (scans
ohne Text schlagen bewusst fehl mit Hinweis), Bilder → Base64 (braucht Vision-Modell). Erzwingt
...
zwischen fünf Seiten (`pages/`: Stammdaten, Kosten, Zaehler, Abrechnung, Einstellungen).
